...
export const InvoicesPage = () => {
  ...
  const doctors = Array.from(
    new Map(
      invoicesData.map((invoice) => [
        invoice.doctor.id,
        { ...invoice.doctor, avatarUrl: invoice.doctor.avatarUrl },
      ])
    ).values()
  )

  const filteredInvoices = selectedDoctor
    ? invoicesData.filter((invoice) => invoice.doctor.id === selectedDoctor)
    : invoicesData
  ...
  return (
    <div>
      <Container header={t('invoices.page.header')} button="Krijo Faturë">
