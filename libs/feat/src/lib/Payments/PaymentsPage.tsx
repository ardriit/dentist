...
export const PaymentsPage = () => {
  ...
  const doctors = Array.from(
    new Map(
      paymnetsData.map((payment) => [
        payment.doctor.id,
        { ...payment.doctor, avatarUrl: payment.doctor.avatarUrl },
      ])
    ).values()
  )

  const filteredPayments = selectedDoctor
    ? paymnetsData.filter((payment) => payment.doctor.id === selectedDoctor)
    : paymnetsData
  ...
  return (
    <div>
      <Container header={t('payments.page.header')}>
        ...
        <MyTable
          tableHead={paymnetsFields}
          tableData={filteredPayments.map((payment) => ({
            ...payment,
